...
if not st.session_state.authenticated:
    ...
    # Hero texts + stats (keep your exact code)
    ...
    # Realtime Stats (your exact code)
    try:
...
*Built by faith, tested by fire.*
    """)
        ...
        # Realization & Vision (nasa loob na ng full journey — last section bago ang Close button)
    st.markdown(
        f"<h3 style='color:{accent_gold}; text-align:center; font-size:1.8rem; margin:2rem 0;'>"
    ...
    # No cropping — direct full original image (buong-buo, preserves aspect ratio, responsive)
    st.image(
...
— Mark Jeff Blando | Founder & Developer | 2014 hanggang ngayon 👑
    ...
    st.markdown("</div>", unsafe_allow_html=True)  # Close full journey card (glass-card)
...
with st.sidebar:
    ...
    st.markdown(
        f"<p style='text-align:center; color:{accent_primary};'><strong>{st.session_state.role.title()}</strong></p>",
        unsafe_allow_html=True,
    )
    st.divider()
    ...
    if current_role in ["owner", "admin"]:
        ...
        with st.expander("➕ Launch New FTMO Account", expanded=True):
            with st.form("create_account_form", clear_on_submit=True):
                ...
                # ── THIS IS THE LINE THAT FIXES THE "MISSING SUBMIT BUTTON" WARNING ──
            ...
            if "edit_acc_id" in st.session_state:
                ...
                with st.expander(f"✏️ Editing {cur['name']}", expanded=True):
                    with st.form(f"edit_form_{eid}", clear_on_submit=True):
                        ...
                        with col_save:
                            # ── THIS IS THE LINE THAT FIXES THE "MISSING SUBMIT BUTTON" WARNING FOR EDIT FORM ──
